...
class PDMObservation:
    ...
    def __init__(
        ...
        # for setting velocity predictions
        self._velocity_ratios = {}
    ...
    def set_velocity_ratio(self, token, ratio):
        """
        Set velocity ratio used in prediction for vehicle with given token
        """
        self._velocity_ratios[token] = ratio
    ...
    def update(
        ...
        for sample in np.arange(
            ...
            if has_dynamic_object:
                # Instead of assuming one constant velocity, consider multiple velocity profiles
                dynamic_object_polygons = []
                const_vel_ratios = [0.8, 1.0]
                for ratio in const_vel_ratios:
                    delta_t = float(sample) * self._sample_interval
                    ratio_tensor = np.ones(dynamic_object_dxy.shape[0]) * ratio
                    
                    # check for manually set velocities
                    for token in self._velocity_ratios:
                        if token in dynamic_object_tokens:
                            index = dynamic_object_tokens.index(token)
                            ratio_tensor[index] = self._velocity_ratios[token]

                    dxdy = delta_t * dynamic_object_dxy * ratio_tensor[:,None]
                    dynamic_object_coords_t = dynamic_object_coords + dxdy[:,None]
                    dynamic_object_polygons.append(
                        shapely.creation.polygons(
                            dynamic_object_coords_t
                        )
                    )
                dynamic_object_polygons = np.concatenate(dynamic_object_polygons, axis=0)
                dynamic_object_tokens_t = list(chain(*[dynamic_object_tokens for _ in range(len(const_vel_ratios))]))
            else:
                dynamic_object_tokens_t = []
            ...
            occupancy_map = PDMOccupancyMap(
                static_object_tokens + dynamic_object_tokens_t + traffic_light_tokens,
                all_polygons,
            )
