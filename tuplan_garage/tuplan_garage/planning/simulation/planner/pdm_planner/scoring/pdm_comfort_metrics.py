...
# for converting rear axle to center
...
def _extract_ego_acceleration(
    ...
    if acceleration_coordinate == "x" or acceleration_coordinate == "y":
        rear_axle_acc = states[..., [StateIndex.ACCELERATION_X, StateIndex.ACCELERATION_Y]]
        angular_vel = states[..., [StateIndex.ANGULAR_VELOCITY]]
        angular_acc = states[..., [StateIndex.ANGULAR_ACCELERATION]]
        displacement = np.array([rear_axle_to_center, 0])[None,None]

        centripetal_acc_term = displacement * angular_vel**2
        angular_acc_term = displacement * angular_acc
        center_acc = rear_axle_acc + centripetal_acc_term + angular_acc_term

        if acceleration_coordinate == "x":
            acceleration = center_acc[...,0]
        else:
            acceleration = center_acc[...,1]
...
def _compute_lon_acceleration(
    ...
    return _within_bound(
        lon_acceleration, min_bound=comfort_config['min_lon_accel'], max_bound=comfort_config['max_lon_accel']
    )
...
def _compute_lat_acceleration(
    ...
    return _within_bound(
        lat_acceleration, min_bound=-comfort_config['max_abs_lat_accel'], max_bound=comfort_config['max_abs_lat_accel']
    )
...
def _compute_jerk_metric(
    ...
    return _within_bound(
        jerk_metric, min_bound=-comfort_config['max_abs_mag_jerk'], max_bound=comfort_config['max_abs_mag_jerk']
    )
...
def _compute_lon_jerk_metric(
    ...
    return _within_bound(
        lon_jerk_metric, min_bound=-comfort_config['max_abs_lon_jerk'], max_bound=comfort_config['max_abs_lon_jerk']
    )
...
def _compute_yaw_accel(
    ...
    return _within_bound(
        yaw_accel_metric, min_bound=-comfort_config['max_abs_yaw_accel'], max_bound=comfort_config['max_abs_yaw_accel']
    )
...
def _compute_yaw_rate(
    ...
    return _within_bound(
        yaw_rate_metric, min_bound=-comfort_config['max_abs_yaw_rate'], max_bound=comfort_config['max_abs_yaw_rate']
    )
...
def ego_is_comfortable(
    ...
    for idx, metric_function in enumerate(comfort_metric_functions):
        results[:, idx] = metric_function(states, time_point_s, comfort_config)
