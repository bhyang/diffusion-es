...
class PDMGenerator:
        ...
        if not update_leading_agent:
            ...
            for proposal_idx in lateral_batch_idcs:
                ...
                if len(agents_ahead) > 0:  # red light, object or agent ahead
                    ...
                    # save nearest agent (only at first step)
                    if time_idx == 2:
                        # we only care about certain types
                        if nearest_agent in self._observation.unique_objects:
                            agent_object = self._observation.unique_objects[nearest_agent]
                            if (relative_distances[argmin] < self._lead_agent_dist) and \
                                (agent_object.tracked_object_type in AGENT_TYPES):
                                self._lead_agent = nearest_agent
                                self._lead_agent_dist = relative_distances[argmin]
    ...
    def get_max_speed(self, dt=.1):
        positions = self._state_array[:,:,[StateIndex.X, StateIndex.Y]]
        deltas = np.linalg.norm(np.diff(positions, axis=1), axis=-1)
        speeds = deltas / dt
        return speeds.max()

    def dist_to_leading_vehicle(self):
        lead_progress = self._leading_agent_array[:, 2, LeadingAgentIndex.PROGRESS]
        lead_rear_length = self._leading_agent_array[:, 2, LeadingAgentIndex.LENGTH_REAR]
        # Also need the ego progress to get relative dists
        current_ego_progress = self._state_idm_array[:, 1, StateIDMIndex.PROGRESS]
        relative_dist = lead_progress - current_ego_progress - lead_rear_length
        return relative_dist.min()
