...
class PDMPath:
    ...
    def get_nearby_path(self, ego_state, speed_limit=None, min_dist=25.0):
        if speed_limit is None:
            max_dist = min_dist
        else:
            max_dist = max(speed_limit * 4.0, min_dist)

        start_dist = self.project(
            Point((ego_state.center.x, ego_state.center.y)))
        centerline = self.substring(start_dist, start_dist + max_dist)
        return centerline