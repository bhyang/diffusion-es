...
class PDMSimulator:
    ...
    def simulate_proposals(
        ...
        # also optionally track command states
        self._commands = np.zeros((proposal_states.shape[0], proposal_states.shape[1]-1, 2), dtype=np.float64)
        ...
        for time_idx in range(1, self._proposal_sampling.num_poses + 1):
            ...
            self._commands[:, time_idx - 1] = command_states
