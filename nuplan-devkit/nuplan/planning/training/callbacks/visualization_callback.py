...
class VisualizationCallback(pl.Callback):
    ...
    def _initialize_dataloaders(self, datamodule: pl.LightningDataModule) -> None:
        ...
        if isinstance(val_loaders, list):
            val_set = val_loaders[1].dataset
        else:
            val_set = val_loaders.dataset
    ...
    def _log_from_dataloader(
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            for key in targets: features[key] = targets[key]
    ...
    def _log_batch(
        ...
        if 'raster' in features:
            ...
            'agent_history' in features
        ):
            image_batch = self._get_images_from_new_vector_features(features, targets, predictions)
        else:
            return
    ...
    def _get_images_from_vector_features(
        ...
        visualize_diffusion = 'multimodal_trajectories' in predictions
        # if visualize_diffusion:
        #     diffusion_trajectories = predictions['diffusion_trajectories'].trajectories
        #     diffusion_trajectories = [trajs.unpack() for trajs in diffusion_trajectories]

        for idx, (vector_map, agents, target_trajectory, predicted_trajectory) in enumerate(zip(
            features[vector_map_feature].unpack(),
            features[agents_feature].unpack(),
            targets['trajectory'].unpack(),
            predictions['trajectory'].unpack(),
        )):
            if not visualize_diffusion:
                ...
                predicted_trajectories = [
                    Trajectory(data=predictions['multimodal_trajectories'][idx:idx+1,i]) \
                    for i in range(predictions['multimodal_trajectories'].shape[1])
                ]
                image = get_raster_from_vector_map_with_agents(
                    vector_map,
                    agents,
    ...
    def _get_images_from_new_vector_features(
        self, features: FeaturesType, targets: TargetsType, predictions: TargetsType
    ) -> npt.NDArray[np.uint8]:
        images = list()
        vector_map_feature = 'vector_map' if 'vector_map' in features else 'vector_set_map'
        ...
        for idx, (
            ...
            features[vector_map_feature].unpack(),
            features['agent_history'].unpack(),
            targets['trajectory'].unpack(),
            predictions['trajectory'].unpack(),
            # predictions['agent_trajectories'].unpack()
        )):
            if 'multimodal_trajectories' in predictions:
                multimodal_trajectories = predictions['multimodal_trajectories'][idx]
                multimodal_trajectories = multimodal_trajectories.reshape(-1,16,3)
            else:
                multimodal_trajectories = None

            image = get_raster_from_vector_map_with_new_agents(
                vector_map,
                agent_history,
                ego_trajectory,
                pred_ego_trajectory=predicted_ego_trajectory,
