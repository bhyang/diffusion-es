...
def create_dataset(
    ...
    return ScenarioDataset(
        ...
        load_single_sample=load_single_sample
    )
...
class DataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        self._val_dataloader_params = val_dataloader_params
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if stage == 'fit' or stage == 'validate':
            ...
            self._train_set = create_dataset(
                ...
                "train",
                self._augmentors,
                load_single_sample=self._load_single_sample
            )
    ...
    def val_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        val_params = deepcopy(self._dataloader_params)
        val_params.update(self._val_dataloader_params)

        val_loader = torch.utils.data.DataLoader(
            dataset=self._val_set, **val_params, collate_fn=FeatureCollate()
        )
        train_loader = torch.utils.data.DataLoader(
            dataset=self._train_set, **val_params, collate_fn=FeatureCollate()
        )

        # so we can do val metrics but on the train set; useful for diffusion only
