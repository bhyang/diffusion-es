...
class LightningModuleWrapper(pl.LightningModule):
    ...
    def _step(self, batch: Tuple[FeaturesType, TargetsType, ScenarioListType], prefix: str, sync_dist: bool) -> torch.Tensor:
        """
        ...
        self._log_step(loss, objectives, metrics, prefix, sync_dist)
    ...
    def _compute_objectives(
        ...
        """
        vals = {objective.name(): objective.compute(predictions, targets, scenarios) for objective in self.objectives}
        return vals
    ...
    def _compute_metrics(self, predictions: TargetsType, targets: TargetsType) -> Dict[str, torch.Tensor]:
        ...
        """
        vals = {metric.name(): metric.compute(predictions, targets) for metric in self.metrics}
        return vals
    ...
    def _log_step(
        ...
        """
        self.log(f'loss/{prefix}_{loss_name}', loss, add_dataloader_idx=False, sync_dist=sync_dist)

        for key, value in objectives.items():
            self.log(f'objectives/{prefix}_{key}', value, add_dataloader_idx=False, sync_dist=sync_dist)

        for key, value in metrics.items():
            self.log(f'metrics/{prefix}_{key}', value, add_dataloader_idx=False, sync_dist=sync_dist)
    ...
    def training_step(self, batch: Tuple[FeaturesType, TargetsType, ScenarioListType], batch_idx: int) -> torch.Tensor:
        ...
        """
        return self._step(batch, 'train', sync_dist=False)

    def validation_step(
        self, batch: Tuple[FeaturesType, TargetsType, ScenarioListType], batch_idx: int, dataloader_idx: int
    ) -> torch.Tensor:
        """
        ...
        """
        if dataloader_idx == 0:
            return self._step(batch, 'trainval', sync_dist=True)
