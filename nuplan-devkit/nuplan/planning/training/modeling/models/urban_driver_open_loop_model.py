...
class UrbanDriverOpenLoopModel(TorchModuleWrapper):
    ...
    def __init__(
        ...
        self.standardize = self._feature_params.standardize
    ...
    def extract_agent_features(
        self, ego_agent_features: GenericAgents, batch_size: int, return_positions: bool = False
        ...
        for sample_idx in range(batch_size):
            ...
            sample_positions = [sample_ego_feature[:,0,:2]]
            ...
            for feature_name in self._feature_params.agent_features:
                ...
                if ego_agent_features.has_agents(feature_name, sample_idx):
                    ...
                    sample_agent_positions = sample_agent_features[:,0,:2]
                    ...
                    sample_agent_positions = torch.zeros(
                        self._feature_params.max_agents,
                        2,
                        dtype=torch.float32,
                        device=sample_ego_feature.device
        ...
        if return_positions:
            return agent_features, agent_avails, agent_positions
        else:
            return agent_features, agent_avails

    def extract_map_features(
        self, vector_set_map_data: VectorSetMap, batch_size: int, return_positions: bool = False
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        for sample_idx in range(batch_size):
            ...
            for feature_name in self._feature_params.map_features:
                ...
                sample_map_positions.append(positions)

            map_features.append(torch.cat(sample_map_features))
            map_avails.append(torch.cat(sample_map_avails))
            map_positions.append(torch.cat(sample_map_positions))

        map_features = torch.stack(map_features)
        map_avails = torch.stack(map_avails)
        map_positions = torch.stack(map_positions)

        if return_positions:
            return map_features, map_avails, map_positions
        else:
    ...
    def forward(self, features: FeaturesType) -> TargetsType:
        ...
        if self.standardize:
            ...
            cutoff_ratio = self.max_token_dist / self.max_dist
