...
def pad_agent_states(agent_trajectories: List[torch.Tensor], reverse: bool) -> List[torch.Tensor]:
    ...
    for idx in range(len(agent_trajectories)):
        ...
        for row_idx in range(frame.shape[0]):
            row_idx_ = int(frame[row_idx, track_id_idx].item())
            if row_idx_ in id_row_mapping:
                mapped_row: int = id_row_mapping[row_idx_]
                current_state[mapped_row, :] = frame[row_idx, :]
            else:
                print('Found future vehicle that doesnt appear in present timestep')
...
def filter_agents_tensor(agents: List[torch.Tensor], reverse: bool = False, ref_idx: int = 0) -> List[torch.Tensor]:
    """
...
def extract_track_token_ids(agents: List[Agent]) -> Dict[str, int]:
    track_token_ids = {agent.track_token: idx for (idx, agent) in enumerate(agents)}
    return track_token_ids


def build_history_features(
    history_tracked_objects: List[TrackedObjects], 
    track_token_ids: Dict[str, int],
    ego_state: EgoState,
    object_type: TrackedObjectType = TrackedObjectType.VEHICLE
) -> Tuple[torch.Tensor, torch.Tensor]:
    num_timesteps = len(history_tracked_objects)
    num_agents = len(track_token_ids)

    data = torch.zeros(num_timesteps, num_agents, 7)
    mask = torch.zeros(num_timesteps, num_agents, dtype=torch.bool)

    for t in range(num_timesteps):
        agents = history_tracked_objects[t].get_tracked_objects_of_type(object_type)
        for agent in agents:
            if agent.track_token in track_token_ids:
                idx = track_token_ids[agent.track_token]
                data[t, idx] = torch.as_tensor((
                    agent.center.x,
                    agent.center.y,
    ...
    flat_data = data.reshape(-1,7).double()

    positions = flat_data[:,:3]
    velocities = torch.cat([flat_data[:,3:5], torch.zeros(flat_data.shape[0],1)], dim=1)
    transformed_positions = global_state_se2_tensor_to_local(positions, ego_pose, precision=torch.float64)
    transformed_velocities = global_state_se2_tensor_to_local(velocities, ego_velocity, precision=torch.float64)

    flat_data[:,:3] = transformed_positions
    flat_data[:,3:5] = transformed_velocities[:,:2]

    data = flat_data.reshape(data_shape).float()

    return data, mask


def build_trajectory_features(
    trajectory_tracked_objects: List[TrackedObjects], 
    track_token_ids: Dict[str, int],
    ego_state: EgoState,
    object_type: TrackedObjectType = TrackedObjectType.VEHICLE
) -> Tuple[torch.Tensor, torch.Tensor]:
    num_timesteps = len(trajectory_tracked_objects)
    num_agents = len(track_token_ids)

    data = torch.zeros(num_timesteps, num_agents, 3)
    mask = torch.zeros(num_timesteps, num_agents, dtype=torch.bool)

    for t in range(num_timesteps):
        agents = trajectory_tracked_objects[t].get_tracked_objects_of_type(object_type)
        for agent in agents:
            if agent.track_token in track_token_ids:
                idx = track_token_ids[agent.track_token]
                data[t,idx] = torch.as_tensor((
    ...
    data_shape = data.shape
    flat_data = data.reshape(-1,3).double()

    flat_data = global_state_se2_tensor_to_local(flat_data, ego_pose, precision=torch.float64)
