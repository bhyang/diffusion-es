...
class VectorSetMap(AbstractModelFeature):
    ...
    def collate(cls, batch: List[VectorSetMap]) -> VectorSetMap:
        ...
        # Because of shared memory constraints / too many memory segments otherwise
        for feature_name in coords:
            coords[feature_name] = torch.stack(coords[feature_name])
        
        for feature_name in traffic_light_data:
            traffic_light_data[feature_name] = torch.stack(traffic_light_data[feature_name])

        for feature_name in availabilities:
            availabilities[feature_name] = torch.stack(availabilities[feature_name])
