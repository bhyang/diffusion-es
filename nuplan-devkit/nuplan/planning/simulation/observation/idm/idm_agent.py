...
class IDMAgent:
    ...
    def propagate(self, lead_agent: IDMLeadAgentState, tspan: float) -> None:
        ...
        if self._behavior_policy != 'static':
            if self._noisy:
                # every so often, mix things up
                if np.random.rand() > .98:
                    self._policy = IDMPolicy(
                        target_velocity=20, # np.random.uniform(0,20),
                        min_gap_to_lead_agent=0.1, # np.random.uniform(0.5,1.5),
                        headway_time=1.5,
                        accel_max=5.0,
                        decel_max=5.0,
                    )
