...
class EgoIsComfortableStatistics(MetricBase):
    ...
    def check_ego_is_comfortable(self, history: SimulationHistory, scenario: AbstractScenario) -> bool:
        ...
        all_metrics_results = [metric.within_bound_status_history for metric in self._comfortability_metrics]
        ego_is_comfortable_history = np.stack(all_metrics_results, axis=0).all(axis=0)

        return ego_is_comfortable, ego_is_comfortable_history
    ...
    def compute(self, history: SimulationHistory, scenario: AbstractScenario) -> List[MetricStatistics]:
        ...
        """
        ego_is_comfortable, ego_is_comfortable_history = self.check_ego_is_comfortable(history=history, scenario=scenario)
        statistics = [
            Statistic(
        ...
        # ego_is_comfortable_history
        return results, ego_is_comfortable_history
