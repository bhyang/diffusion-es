...
class EgoProgressAlongExpertRouteStatistics(MetricBase):
    ...
    def compute(self, history: SimulationHistory, scenario: AbstractScenario) -> List[MetricStatistics]:
        ...
        if not expert_route_simplified:
            ...
            self.ego_progress = ego_progress_computer(ego_poses=ego_poses)
            overall_ego_progress = np.sum(self.ego_progress)
            ...
            self.expert_progress = expert_progress_computer(ego_poses=expert_poses)
            overall_expert_progress = np.sum(self.expert_progress)
            ...
            time_series = TimeSeries(unit='meters', time_stamps=list(ego_timestamps), values=list(self.ego_progress))
            statistics = [
                Statistic(
                    name='expert_total_progress_along_route',
                    unit='meters',
        ...
        # self.ego_progress, self.expert_progress
        return self.results, (
            self.ego_progress, self.expert_progress
        )
